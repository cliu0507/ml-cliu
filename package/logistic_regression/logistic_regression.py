    ...
    for i in range(0,numIterations):
        ...
        for j in range(0,theta.shape[0]):
            ...
            for k in range(0,m):
                ...
                error += ((-1)* y[k][0] * np.log(hypothesis[k][0]) - (1-y[k][0])*np.log(1-hypothesis[k][0]))/m
...
def predict(theta, x_scaled):
    y_predict=1/(1-np.exp((-1)*(np.dot(theta.transpose(),x_scaled.transpose()))))
    return y_predict
...
def main():
    ...
    x_file=data_dir+'/classification_example.csv'
